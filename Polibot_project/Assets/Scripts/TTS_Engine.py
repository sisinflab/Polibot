...
CI_ITA = CultureInfo("it-IT")

# I first try to select the specific voice I'm looking for, if the selection fails I try to take an italian masculine
# voice and if this fails I try to take an italian voice. If all these attempts fail the server will use
# the first voice available in the system regardless of gender and nationality.
try:
    synth.SelectVoice('Microsoft Cosimo')
except Exception as e:
    print(e)
    try:
        synth.SelectVoiceByHints(VoiceGender.Male, CI_ITA)
    except Exception as e:
        print(e)
        try:
            synth.SelectVoiceByHints(CI_ITA)
        except Exception as e:
            print(e)
